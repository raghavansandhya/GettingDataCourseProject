...
```
There are a total of `r dim(train_sub)[1]` obs. in Training Dataset and `r dim(test_sub)[1]` obs. in Test Dataset.
...
2. Merged the combined_test and combined_training data **row-wise** to create the complete dataset. The complete dataset now has `r dim(train_sub)[1] + dim(test_sub)[1]` obs and 563 columns
